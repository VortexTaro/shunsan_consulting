...
if prompt := st.chat_input("質問や相談したいことを入力してね"):
    ...
    with st.chat_message("assistant", avatar=AVATAR_IMAGE_PATH):
        ...
        with st.spinner("思考中..."):
            ...
            # AIの役割を「着眼点シフトモード」に固定し、関連情報を参考として渡す
            final_prompt = f"""
# 君の役割: 着眼点シフトモード
...
2.  次に、その事象の裏にある本質的なテーマ（例：価値の受け取り方、自己肯定感、理想の世界観）を、君が持つナレッジベース全体や、以下の「参考情報」から見抜きます。
...
---
# 参考情報
以下の情報は、君が新しい着眼点を提示するためのヒントです。もし役立つ情報があれば、積極的に活用してください。情報がない場合は、君自身の知識で応答してください。
---
{context if context else "今回の対話では、参考となる特定の情報はありません。"}
---
